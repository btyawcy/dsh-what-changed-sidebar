  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        goneBadge: "已删除",
        goneMovedBadge: "已移动",
        goneBodyDelete: "此文件已在第 {turn} 轮被 shell 命令删除（由命令推断，best-effort）",
        goneBodyMove: "此文件已在第 {turn} 轮被 shell 命令移动到 {to}",
        noTextDiff: "该工具的修改不提供文本对比（结构化/二进制格式）",
      },
      en: {
        ...
        retry: "Retry",
        goneBadge: "Deleted",
        goneMovedBadge: "Moved",
        goneBodyDelete: "This file was deleted by a shell command in turn {turn} (best-effort inference)",
        goneBodyMove: "This file was moved to {to} by a shell command in turn {turn}",
        noTextDiff: "This tool's changes provide no textual diff (structured/binary format)",
      }
    };
    ...
    function groupByTurn(files, shells) {
      ...
      (files || []).forEach(function (file) {
        (file.edits || []).forEach(function (edit) {
          var f = ensureFile(ensureTurn(edit.turn), file.path, file.sensitive === true);
          f.edits.push(edit);
          if (file.gone !== void 0) f.gone = file.gone; // 历史块也带"已删除"角标
        });
        (file.refusals || []).forEach(function (r) {
          var f = ensureFile(ensureTurn(r.turn), file.path, file.sensitive === true);
          f.refusals.push(r);
          if (file.gone !== void 0) f.gone = file.gone;
        });
        // 纯删除（无编辑无被拒）：在删除发生的轮次生成一个独立块
        if (file.gone !== void 0 && (file.edits || []).length === 0 && (file.refusals || []).length === 0) {
          ensureFile(ensureTurn(file.gone.turn), file.path, file.sensitive === true).gone = file.gone;
        }
      });
    ...
    function EditBlocks(props) {
      ...
      if (edit.noText) {
        // 无文本对比的工具（univer 等结构化格式）：中性提示，不发请求。
        blocks.push(createElement("div", { className: "wc-gone", key: "notext" }, t(ctx, "noTextDiff")));
        return createElement("div", { className: "wc-edit" }, blocks);
      }
      ...
      if (content === void 0 || content === null) {
        blocks.push(createElement("div", { className: "wc-loading", key: "load" },
          props.loadFailed
            ? createElement("span", { className: "wc-retryWrap" },
                t(ctx, "loadFailed") + " ",
                createElement("button", {
                  type: "button",
                  className: "wc-retryBtn",
                  onClick: function () { if (typeof props.onRetry === "function") props.onRetry(); },
                  title: t(ctx, "retry")
                }, t(ctx, "retry"))
              )
            : "…"
    ...
    // 失败可点击重试：浏览器在 dsh web 重启窗口期发的请求会网络级失败，
    // 失败被缓存后不该永久卡死——"内容加载失败"本身就是重试按钮。
    function ExpandedEdit(props) {
      ...
      var bumpRetry = retryState[1];

      useEffect(function () {
        if (edit.sensitive || edit.noText) { setLoadData(void 0); return; }
      ...
      }, [edit.callId, sessionId, edit.sensitive, filePath, retryCount]);
    ...
    function FileGroup(props) {
      ...
      var setLoaded = loadState[1];
      useEffect(function () {
        if (open !== true || roundEnded !== true || sensitive || editCount === 0 || file.gone !== void 0 || loaded !== void 0) return;
        var alive = true;
      ...
      return createElement("div", { className: "wc-file" },
        ...
        createElement("div", { className: "wc-fileRow" },
          createElement("button", {
            ...
            file.gone !== void 0 && createElement("span", { className: "wc-goneBadge" },
              file.gone.kind === "rename" ? t(ctx, "goneMovedBadge") : t(ctx, "goneBadge")),
        ...
        open && createElement("div", { className: "wc-fileBody" },
          file.gone !== void 0 && createElement("div", { className: "wc-gone" },
            "🗑 " + (file.gone.kind === "rename"
              ? t(ctx, "goneBodyMove", { turn: file.gone.turn, to: file.gone.to !== void 0 ? file.gone.to : "?" })
              : t(ctx, "goneBodyDelete", { turn: file.gone.turn }))),
    ...
    var STYLES = [
      ...
      ".wc-goneBadge{display:inline-flex;align-items:center;padding:1px 8px;border-radius:999px;font-size:.75em;font-weight:600;background:color-mix(in srgb,var(--dsw-alias-state-danger-primary,#d93025) 12%,transparent);color:var(--dsw-alias-state-danger-primary,#a12020)}",
      ".wc-gone{padding:6px 10px;border-radius:6px;font-size:.8em;color:var(--dsw-alias-label-secondary,inherit);background:var(--dsw-alias-interactive-bg,rgba(0,0,0,.04))}",
      ".wc-retryWrap{display:inline-flex;align-items:center;gap:6px;flex-wrap:wrap}",
      ".wc-retryBtn{border:1px solid var(--dsw-alias-border-line,rgba(0,0,0,.15));border-radius:6px;background:transparent;color:var(--dsw-alias-label-primary,inherit);font:inherit;font-size:.9em;padding:1px 8px;cursor:pointer}",
      ".wc-retryBtn:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,.08))}",
