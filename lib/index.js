...
function apply(state, event) {
  ...
  if (event.type === "tool/call") {
    ...
    if (pendingWrite === void 0) return state;
...
var whatChangedProjection = {
  ...
  view,
  // v12: ①pending 阶段敏感路径不再携带全文（中断残留不落盘）；
  //      ②单文件编辑/被拒记录上限（MAX_EDITS_PER_FILE / MAX_REFUSALS_PER_FILE）；
  //      ③view 去掉 of 字段（客户端现算）；④shell 去重与裁剪的计数口径修正。
  stateVersion: 12
};
// 框架在每个"使状态变化的事件"上都会执行 schema.parse(view(state))——这是
// 投影契约的固定成本。zod 深校验 12000 条记录实测 5.92ms/事件，而 view 是
...
function apply2(ctx) {
  // 热路径用浅校验（见 fastSchema 注释）；导出的 whatChangedProjection.schema
  // 仍是完整 zod schema，供测试与离线校验使用。
  ctx.sessionProjections.register({ ...whatChangedProjection, schema: fastSchema });
  ...
  function readDiskCapped(path) {
    try {
      ...
      if (st.size <= SNAP_MAX_BYTES) return readFileSync(path, "utf8");
    } catch (e) { /* 文件不存在等 */ }
