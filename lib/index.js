...
function pruneTurns(state) {
  ...
  if (turns.length > KEEP_TURNS) {
    ...
    for (const [path, file] of Object.entries(state.files)) {
      ...
      if (edits.length === 0 && refusals.length === 0) continue; // 整文件都被裁掉就整个移除
      kept[path] = { path: file.path, edits, refusals, sensitive: file.sensitive };
  ...
  if (event.type === "tool/call") {
    ...
    if (SHELLS.has(callData.name)) {
      ...
      if (command !== void 0 && looksLikeShellWrite(command)) {
        ...
        state.pendingShell[callData.callId] = { turn: callData.turn, tool: callData.name, hash: hashOf(command) };
...
// ── view 结构校验（手写，零依赖）──
// 早期用 zod 构造 view schema，但 zod 是插件唯一的运行时依赖——插件目录以
// 符号链接形式安装时（工作区开发态），Node 沿真实路径解析依赖，zod 不在
// 解析链上会让整个 dsh web 启动失败（2026-08-26 实际事故）。view 由本模块
// 自己构造，手写结构校验足够，从此插件零运行时依赖。
...
function checkEditShape(edit, at) {
  if (!isRecord(edit)) return failParse("edits[" + at + "] must be an object");
  if (typeof edit.turn !== "number") return failParse("edits[" + at + "].turn must be a number");
  if (typeof edit.callId !== "string") return failParse("edits[" + at + "].callId must be a string");
  if (edit.kind !== "edit" && edit.kind !== "create") return failParse("edits[" + at + "].kind must be edit|create");
  if (typeof edit.seq !== "number") return failParse("edits[" + at + "].seq must be a number");
  if (edit.source !== "arguments" && edit.source !== "applied") return failParse("edits[" + at + "].source invalid");
  return { success: true, value: edit };
}
// 完整 view 结构校验（测试/离线校验用；接口对齐 zod 的 safeParse）。
var viewSchema = {
  safeParse(value) {
    if (!isRecord(value)) return failParse("view must be an object");
    if (!Array.isArray(value.files) || !Array.isArray(value.shells)) {
      return failParse("view.files/shells must be arrays");
    }
    if (typeof value.totalEdits !== "number" || typeof value.totalRefused !== "number" ||
        typeof value.shellWrites !== "number" || typeof value.lastEndedTurn !== "number") {
      return failParse("view counters must be numbers");
    }
    for (let i = 0; i < value.files.length; i += 1) {
      const file = value.files[i];
      if (!isRecord(file) || typeof file.path !== "string") return failParse("files[" + i + "].path must be a string");
      if (!Array.isArray(file.edits) || !Array.isArray(file.refusals)) return failParse("files[" + i + "] edits/refusals must be arrays");
      for (let j = 0; j < file.edits.length; j += 1) {
        const result = checkEditShape(file.edits[j], i + ":" + j);
        if (result.success === false) return result;
      }
      for (let j = 0; j < file.refusals.length; j += 1) {
        const refusal = file.refusals[j];
        if (!isRecord(refusal) || typeof refusal.reason !== "string") return failParse("refusals[" + i + ":" + j + "] invalid");
      }
    }
    for (let i = 0; i < value.shells.length; i += 1) {
      const shell = value.shells[i];
      if (!isRecord(shell) || typeof shell.callId !== "string" || typeof shell.repeat !== "number") {
        return failParse("shells[" + i + "] invalid");
      }
    }
...
var whatChangedProjection = {
  ...
  //      ②univer 系列进 STRUCTURED_WRITERS（noText）；③二进制不进快照；
  //      ④pending 敏感路径不带全文；⑤单文件记录上限；⑥view 去 of。
  stateVersion: 14,
  schema: viewSchema,
  view
};
...
function apply2(ctx) {
  ...
  // ── 绝对行号补全 ──
  // 某些 DSH 版本的 applied diff 不带 oldStart/newStart（0.1.1-rc.2 的 edit
  // 工具实测就没有）——没有行号，客户端只能显示"块内相对行号"。
  // 补全办法：改前快照里 oldText 唯一出现时，出现位置就是精确的旧行号，
  // 新文本起始于同一行（前缀未变）。歧义（出现多次）或无快照时保持相对，
  // 绝不猜。内存热快照优先，落盘文档的 blob 兜底。
  function countTextLines(text) {
    return text === "" ? 0 : text.split("\n").length;
  }
  function countOccurrencesOf(hay, needle) {
    if (needle === "") return 0;
    let count = 0;
  ...
  function deriveStarts(beforeContent, oldText, newText) {
    if (typeof beforeContent !== "string" || typeof oldText !== "string" || oldText === "") return void 0;
    if (countOccurrencesOf(beforeContent, oldText) !== 1) return void 0;
    const at = beforeContent.indexOf(oldText);
    const oldStart = beforeContent.slice(0, at).split("\n").length;
    return {
      oldStart,
      oldLines: countTextLines(oldText),
      ...(typeof newText === "string" ? { newStart: oldStart, newLines: countTextLines(newText) } : {})
    };
  }
  function beforeContentOf(sessionId, path, callId) {
    const key = sessionId + "\n" + path;
    const map = SNAPSHOTS.get(key);
    if (map !== void 0 && map.has(callId)) return map.get(callId).c;
    ensureSnapDocLoaded(sessionId);
    const doc = SNAPSHOT_DOCS.get(sessionId);
    if (doc !== void 0) {
      const entry = doc.beforeByCall[callId];
      if (entry !== void 0 && entry.p === path) {
        return typeof entry.b === "string" ? (doc.blobs[entry.b] !== void 0 ? doc.blobs[entry.b] : null) : null;
      }
    }
    return void 0;
  }
  function enrichStarts(sessionId, path, callId, target) {
    if (target.oldStart !== void 0) return; // 已有行号（其他 DSH 版本带）不覆盖
    if (typeof target.oldText !== "string" || target.oldText === "") return;
    const before = beforeContentOf(sessionId, path, callId);
    const derived = before === null || before === void 0 ? void 0 : deriveStarts(before, target.oldText, target.newText);
    if (derived === void 0) return;
    target.oldStart = derived.oldStart;
    target.oldLines = derived.oldLines;
    if (derived.newStart !== void 0) {
      target.newStart = derived.newStart;
      target.newLines = derived.newLines;
    }
  }
  function enrichFragmentStarts(sessionId, path, fragments) {
    for (const fragment of fragments) enrichStarts(sessionId, path, fragment.callId, fragment);
    return fragments;
  }
      ...
      handler: async (req, res) => {
        try {
          ...
          if (wantsWhole) {
            ...
            writeJson(res, 200, {
              ok: true,
              value: { file: fileValue, snapshots: snaps, roundAfter: roundAfter, edits: enrichFragmentStarts(body.sessionId, body.path, collectFileEdits(events, body.path)) }
            });
            return;
          }
          if (wantsEdits) {
            // 该文件的完整片段历史（时间正序）：客户端重建各轮整文件状态的原料。
            writeJson(res, 200, { ok: true, value: { edits: enrichFragmentStarts(body.sessionId, body.path, collectFileEdits(events, body.path)) } });
            return;
          }
          if (wantsCommand) {
          ...
          enrichStarts(body.sessionId, body.path, body.callId, diff);
